...
class TimerVC: UIViewController {
    ...
    var numberOfWorkCompletion: Int = 0
    var mainScreenImg = UIImageView()
    
    let COUNTING_KEY = "countingKey"
    let workSessionRecordCell = WorkSessionsRecordCell()
...
//        }
        ...
        animationView = .init(name: "growing_tree")
        animationView?.frame = view.bounds
        animationView?.translatesAutoresizingMaskIntoConstraints = false
        animationView?.clipsToBounds = true
        view.addSubview(animationView!)
        animationView?.centerXAnchor.constraint(equalTo: changePomodoroStateBtn.centerXAnchor).isActive = true
        animationView?.topAnchor.constraint(equalTo: currentTaskLabel.bottomAnchor, constant: 35).isActive = true
        animationView?.widthAnchor.constraint(lessThanOrEqualToConstant: view.frame.width).isActive = true
        animationView?.heightAnchor.constraint(equalToConstant: 150).isActive = true
        
    }
    func updateNumber() -> Int {
        
        numberOfWorkCompletion += 1
        return numberOfWorkCompletion
    }
    ...
    fileprivate lazy var currentTaskLabel: UILabelWithInsets = {
        ...
        label.text = "Let plant a tree by finish your WORK"
    ...
    fileprivate func setUpViews() {
        ...
        view.addSubview(mainScreenImg)
        mainScreenImg.translatesAutoresizingMaskIntoConstraints = false
        mainScreenImg.image = UIImage(named: "MainScreen")
        NSLayoutConstraint.activate([
            mainScreenImg.topAnchor.constraint(equalTo: view.topAnchor),
            mainScreenImg.bottomAnchor.constraint(equalTo: view.bottomAnchor),
            mainScreenImg.leadingAnchor.constraint(equalTo: view.leadingAnchor),
            mainScreenImg.trailingAnchor.constraint(equalTo: view.trailingAnchor)
        ])
...
//        changePomodoroStateBtn.constrainWidth(constant: 85)
        ...
        NSLayoutConstraint.activate([
            ...
            circularProgressBarParentView.centerYAnchor.constraint(equalTo: view.centerYAnchor, constant: 40),
    ...
    func checkForPermission() {
        ...
        notificationCenter.getNotificationSettings { settings in
            ...
            case .authorized:
                if self.pomodoroSessionType == .work {
                    self.dispatchNotification()

                }
                if self.pomodoroSessionType == .shortBreak {
                    self.dispatchBreakNotification()
                }
            case .denied:
                return
            case .notDetermined:
    ...
    func dispatchNotification() {
        ...
        content.title = "Your work session is completed"
        content.body = "Your tree is growing up to the next stage"
        
        content.categoryIdentifier = "alarm"
        content.userInfo = ["customData": "fizzbuzz"]
        content.sound = UNNotificationSound(named: UNNotificationSoundName(rawValue: "MyNoti.mp3"))
        content.badge = 1
        
        let nextTriggerDate = Calendar.current.date(byAdding: .second, value: workDurationInMinutes * 60, to: Date())!
    ...
    func dispatchBreakNotification() {
        let notificationCenter = UNUserNotificationCenter.current()
        let content = UNMutableNotificationContent()
        notificationCenter.removeAllPendingNotificationRequests()
        
        content.title = "Time to relax is up"
        content.body = "This is work time"
        
        content.categoryIdentifier = "alarm"
        content.userInfo = ["customData": "fizzbuzz"]
        content.sound = UNNotificationSound(named: UNNotificationSoundName(rawValue: "BreakNoti.wav"))
        content.badge = 1
        
        let nextTriggerDate = Calendar.current.date(byAdding: .second, value: shortRestDurationInMinutes * 60 + 1, to: Date())!
...
extension TimerVC {
    ...
    fileprivate func calcRestartTime(start: Date, stop: Date) -> Date {
...
//            if currentTimerStatus == .inactive {
                updateStateAnimation(for: nextFocusBlock)
...
//            }
        ...
        if pomodoroSessionType == .shortBreak {
            self.checkForPermission()
            self.dispatchBreakNotification()
        }
    ...
    fileprivate func changeStateTime() {
        animationView?.pause()
        currentTimerStatus = .inactive
    }
    ...
    func updtWorkSessionCell() {
        workSessionRecordCell.numberOfSession = numberOfWorkCompletion
    }
    ...
    @objc fileprivate func didTapResetTimerBtn() {
        ...
        if pomodoroSessionType == .shortBreak || pomodoroSessionType == .longBreak {
            invalidateTimer()
            currentTimerStatus = .inactive
            elapsedTimeInSeconds = 0
            circularProgressBarView.progressLayer.strokeEnd = 1.0
    ...
    @objc fileprivate func completeTimerAction() {
        invalidateTimer()
        currentTimerStatus = .inactive
        elapsedTimeInSeconds = 0
        circularProgressBarView.progressLayer.strokeEnd = 1.0
    ...
    fileprivate func updateStateAnimation(for currentTimeBlock: FocusSession) {
        switch currentTimeBlock {
        case .firstSession:
            animationView?.play(fromFrame: CGFloat(1), toFrame: CGFloat(27))
            animationView?.animationSpeed = 4 / CGFloat(workDurationInMinutes * minutesToSecondsMultiplier)
        case .secondSession:
            animationView?.play(fromFrame: CGFloat(27), toFrame: CGFloat(72))
            animationView?.animationSpeed = 4 / CGFloat(workDurationInMinutes * minutesToSecondsMultiplier)
        case .thirdSession:
            animationView?.play(fromFrame: CGFloat(72), toFrame: CGFloat(77))
            animationView?.animationSpeed = 4 / CGFloat(workDurationInMinutes * minutesToSecondsMultiplier)
        case .fourthSession:
            animationView?.play(fromFrame: CGFloat(77), toFrame: CGFloat(121))
            animationView?.animationSpeed = 4 / CGFloat(workDurationInMinutes * minutesToSecondsMultiplier)
        }
    }
